...
async def parse_callback(callback: types.CallbackQuery):
    ...
    if callback.message is not None:
        await bot.edit_message_text(
            "Отправьте ссылку на группу, откуда нужно собрать фотографии, сообщения из группы ",
    ...
    async def process_media(message: Message, state: FSMContext) -> None:
        ...
        if media == "yes":
            res, file_name, photo_files = await parser_main(url, limit, media, chat_id)
            ...
            await message.answer(
                f"Сколько фотографий вы хотите отправить? Скачано {total_photos}"
            )

        elif media == "no":
            res, file_name, _ = await parser_main(url, limit, media, chat_id)
...
async def process_photo_count(message: Message, state: FSMContext) -> None:
    ...
    await download(url)
    await dom_all(url)
    await parser_main(url, limit, media, chat_id)

    # Logging user data
    logging.info(
        "User data: url=%s, limit=%s, media=%s, chat_id=%s", url, limit, media, chat_id
    )
